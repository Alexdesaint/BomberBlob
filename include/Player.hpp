...
class Player {
public:
    enum Actions { up = 0, down, right, left, action, numOfActions };

    enum JoystickAxe { x = 0, y, numOfAxes };
    ...
    Player(std::string name, Blob::Color::RGB color) :
        name(std::move(name)), color(color) {}

    void controller(bool c) { control = c; }

    void setJoystickAxe(JoystickAxe a, const float *joy) {
        joystickAxes[a] = joy;
    }

    void setKey(Actions a, const bool *key) { keys[a] = key; }
